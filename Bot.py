...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if raw_text.strip() in ("بررسی مسیر داده", "دیباگ دیتا", "چک دیتا") and user_id in ADMIN_IDS:
        ...
        if not env_value:
            debug_text += (
                "\n\n🚨 *نتیجه:* چون `BOT_DATA_PATH` تنظیم نشده، فایل داده روی مسیر موقتیه و "
                "با هر Redeploy پاک می‌شه. برو به تب Variables سرویس روی Railway و این متغیر رو اضافه کن."
            )
        elif not exists:
            debug_text += (
                "\n\n🚨 *نتیجه:* env var تنظیم شده ولی فایل توی این مسیر پیدا نشد — یعنی Volume "
                "درست mount نشده یا مسیرش با env var یکی نیست. مسیر Mount Path داخل Railway رو "
                "دوباره چک کن که دقیقاً با پوشه‌ی این env var (بدون اسم فایل) یکی باشه."
            )
        else:
            debug_text += "\n\n✅ *نتیجه:* همه‌چیز درست به‌نظر می‌رسه — داده باید پایدار بمونه."
