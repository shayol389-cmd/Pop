...
COUPON_COST_MEDIA = 2            # هزینه‌ی هر عکس/فایل/ویدیو بعد از تموم شدن سهمیه‌ی رایگان روزانه (به کوپن)
...
def main_menu_keyboard(is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    if is_admin:
        buttons.append([InlineKeyboardButton("📁 مدیریت فایل‌ها", callback_data="fmenu")])
...
def build_system_help_text(is_admin: bool) -> str:
    ...
    if is_admin:
        text += (
            ...
            "🔸 *مدیریت فایل‌ها (بازی/عکس/ویدیو) + حذف*\n"
            "بنویس `مدیریت فایل ها` یا از منوی اصلی «📁 مدیریت فایل‌ها» رو بزن تا ۳ دسته "
            "(بازی‌ها، عکس‌ها، ویدیوها) رو جدا ببینی. روی هرکدوم بزن تا نمایشش بدی یا با دکمه‌ی "
...
# ============================================================
FILE_CATEGORY_LABELS = {"games": "🎮 بازی‌ها", "photos": "📸 عکس‌ها", "videos": "🎬 ویدیوها"}
...
def build_file_view_keyboard(category: str, kind: str, key: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🗑 حذف این مورد", callback_data=f"fdel|{kind}|{key}")],
            [InlineKeyboardButton("↩️ بازگشت به لیست", callback_data=f"fcat|{category}")],
        ]
    )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS and user_id in PENDING_PHOTO_UPLOADS:
        ...
        await update.message.reply_text(
            ...
            + build_files_menu_text(data),
            reply_markup=build_files_menu_keyboard(data),
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 0.6) دستور ادمین برای دیدن منوی مدیریت فایل‌ها (بازی‌ها / عکس‌ها / ویدیوها)
    if raw_text.strip() in ("لیست آپلودها", "لیست فایل ها", "لیست فایل‌ها", "مدیریت آپلود", "مدیریت فایل ها", "مدیریت فایل‌ها") and user_id in ADMIN_IDS:
    ...
    # 0.61) میان‌برهای مستقیم به هرکدوم از زیربخش‌های فایل
    direct_category_map = {
    ...
    if raw_text.strip() in direct_category_map and user_id in ADMIN_IDS:
        ...
        if kind == "photo":
            ...
            if file_id:
                await context.bot.send_photo(
                    ...
                    caption=f"📸 کلید: `{key}`",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=build_file_view_keyboard("photos", "photo", key),
                )
        else:
            ...
            if entry:
                ...
                caption = f"{icon} کلید: `{key}`\nنوع: {media_type}"
                view_kb = build_file_view_keyboard(category, "file", key)
                if media_type == "document":
                    await context.bot.send_document(
                        chat_id=query.message.chat_id, document=entry["file_id"],
                        caption=caption, parse_mode=ParseMode.MARKDOWN, reply_markup=view_kb,
                    )
                elif media_type == "video":
