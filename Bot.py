...
LEADERBOARD_SIZE = 10            # چند نفر برتر جایزه بگیرن
...
def main() -> None:
    ...
    if os.environ.get("BOT_DATA_PATH"):
        logger.info("✅ BOT_DATA_PATH تنظیم شده. فایل داده در: %s (این یعنی داده‌ها پایدارن)", DATA_FILE)
    else:
        logger.warning(
            "⚠️ BOT_DATA_PATH تنظیم نشده! فایل داده در مسیر موقتی «%s» ذخیره می‌شه و "
            "با هر Redeploy پاک می‌شه. برای رفع این مشکل، توی Railway یه Volume بساز و "
            "Environment Variable با اسم BOT_DATA_PATH (مثلاً /data/bot_data.json) اضافه کن.",
            DATA_FILE,
        )
